...
#define ASSERT_ERROR_MESSAGE(fn, msg) \
  if (!(fn)) { \
    ...
    fprintf(stderr, "  .gatorgit directory and start over; maybe a previous run\n"); \
...
int is_sane_path(const char* path) {
  ...
  // Only allow modifying files in .gatorgit directory
  const char* prefix = ".gatorgit";
...
static void fs_mkdir(const char* dirname) {
  ...
  ASSERT_ERROR_MESSAGE(is_sane_path(dirname), "dirname is not a valid path within .gatorgit");
...
static void fs_rm(const char* filename) {
  ...
  ASSERT_ERROR_MESSAGE(is_sane_path(filename), "filename is not a valid path within .gatorgit");
...
static void fs_mv(const char* src, const char* dst) {
  ...
  ASSERT_ERROR_MESSAGE(is_sane_path(src), "src is not a valid path within .gatorgit");
  ASSERT_ERROR_MESSAGE(is_sane_path(dst), "dst is not a valid path within .gatorgit");
